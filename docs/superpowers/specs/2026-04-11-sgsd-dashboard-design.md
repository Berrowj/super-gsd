...
**Scope:** sgsd1 (Mission Control), sgsd2 (Narrative Feed), and any net-new panes needed to answer the questions the operator actually has about the orchestrator in real time.
**Related:**
...
the operator cannot tell, at a glance, whether the SGSD orchestrator is actually working. The existing dashboards emit plausible output but fail the one test that matters: they do not produce confident answers to the questions the operator actually has while the orchestrator runs. Specifically:

1. **Liveness is ambiguous.** Dashboard shows counts and waves but not "is anything happening right now". the operator cannot distinguish between (a) orchestrator thinking, (b) orchestrator stuck, (c) orchestrator crashed, (d) orchestrator waiting on something external. He has asked multiple times how to tell, and the current answer is "read the commit time column and make a judgment call" — which is a puzzle, not an answer.
...
4. **Trust is broken.** the operator has seen repeated cases in `project-clarity-erp` where agents report PASS without evidence (silent curl fallback counted as a browser audit, spinner screenshots counted as "page renders", empty API responses counted as "working"). The dashboard currently shows none of this. A phase can close with `verdict: passed` on the dashboard while the actual frontend is broken. The dashboard must surface gate evidence, not just gate verdicts.
...
**The goal of this series:** design a new observability layer that answers the operator’s actual questions with evidence from authoritative sources, not cleverness. When it's done, the operator must be able to look at one screen and answer five questions in under three seconds:
...
## 3. The five questions (working draft — the operator to validate)
...
**Q-A. Signal source for "is claude.exe actually running".** On Windows, `tasklist /fi "imagename eq claude.exe"` or `Get-Process claude`. On WSL, `pgrep claude`. Do we want the dashboard on Windows PowerShell (current), or should we move rendering to WSL where signal sourcing is easier? **the operator decides.**

**Q-B. Single pane or two panes?** Current design has Mission Control + Narrative. The five questions might fit in one pane if dense, or two if airy. Which do you prefer? **the operator decides.**

**Q-C. Do we keep the Haiku narrative at all?** It was a nice-to-have but burns tokens. If the agent roster from ORCHESTRATOR-LIVE.jsonl is good enough, we might drop the Haiku call entirely. **the operator decides after Session 2 wireframe.**

**Q-D. Liveness refresh cadence.** Current sgsd1 polls every 10s. For a true liveness indicator we want the "seconds since last activity" counter to tick every second, which means the pane either (a) uses a FileSystemWatcher and only re-renders the counter block, (b) does a 1s loop but writes only the counter line, or (c) accepts 10s granularity and writes "10-20s" bucket ranges. **Recommendation: (a) or (b). the operator confirms.**
...
**Q-F. The one thing you wish it told you.** The single answer to this question, written in plain English, becomes the headline of the new dashboard. Everything else is supporting detail. **the operator writes this.**

---

## 7. Session plan

- **Session 1 (this doc):** Problem, non-goals, five questions, data contract, open questions. **Output: this file.** the operator reviews, edits, answers Q-A through Q-F inline.
- **Session 2:** Wireframe. Based on Session 1 answers, I produce a single ASCII mockup of the target layout. the operator marks it up. We iterate until it's right.
...
- **Session 4:** Implementation. I replace sgsd1/2 with a new script (or two) that implements the wireframe against the data contract. the operator tests live in project-clarity-erp.
...
- A general-purpose tool. The dashboard serves the operator’s workflow on the operator’s two projects. It does not need to be portable, themable, or "productized".
