...
<fix_schema_mode>
## Fix-Schema Mode — triggered by `--fix-schema` flag

This mode repairs a malformed PLAN.md that failed validate.cjs (exit 1). It does NOT create
a new plan; it surgically fixes schema-level violations while preserving all semantic content.

### TRIGGER

`--fix-schema` flag present in invocation context. When triggered, skip all standard planning
steps (discovery, dependency graph, wave assignment, etc.) and follow ONLY this section.

### INPUTS (provided by the orchestrator in the dispatch prompt)

| Field | Description |
|-------|-------------|
| `plan_file_path` | Absolute path to the malformed PLAN.md |
| `error_envelope` | Inline JSON — the most recent `plan-errors.jsonl` row for this plan |
| `schema_path` | `super-gsd/templates/plan-schema-v2.json` — the canonical schema contract |
| `locked_fields` | `{ id, hypothesis, files_touched }` — extracted VERBATIM from the original plan by the orchestrator before dispatch |
| `attempt_K` | Integer 1, 2, or 3 — which attempt this is |

### LOCKED CONSTRAINT (D-09)

**The planner MUST NOT change `task.id`, `task.hypothesis`, or `task.files_touched`.**

These values are locked to what the orchestrator extracted from the original plan.
- If `error_envelope` reports a missing field: **ADD** it with a reasonable default value.
- If `error_envelope` reports a wrong type: **COERCE** to the correct type.
- If `error_envelope` reports field ordering: **REORDER** as required.
- **DO NOT** regenerate, paraphrase, or rephrase `task.hypothesis` or `task.id`.
- **DO NOT** rewrite the plan from scratch — make the minimum edit that resolves each ajv error.

Violations of this constraint: the orchestrator diffs the fix attempt against `locked_fields`
and treats any changed `task.id`, `task.hypothesis`, or `task.files_touched` as a **failed attempt**.

### PROCESS

1. Read the malformed `plan_file_path` in full.
2. Read `schema_path` (`super-gsd/templates/plan-schema-v2.json`) to understand required fields and types.
3. Parse `error_envelope` — extract each ajv error object (`instancePath`, `keyword`, `message`).
4. For each error in the envelope:
   - Locate the failing field path in the YAML frontmatter.
   - Apply the minimum fix: add missing field with default, coerce wrong type, reorder field.
   - Verify `task.id`, `task.hypothesis`, and `task.files_touched` are unchanged after applying the fix.
5. Self-check before writing (mandatory):
   - [ ] `task.id` unchanged (character-for-character match with `locked_fields.id`)
   - [ ] `task.hypothesis` unchanged (character-for-character match with `locked_fields.hypothesis`)
   - [ ] `task.files_touched` unchanged (same array, same order, as `locked_fields.files_touched`)
   - [ ] Every ajv error from `error_envelope` is addressed in the fix
   - [ ] No new fields were added beyond what the schema requires
6. Write the repaired plan to the **sibling file** per D-11:
   `{plan_file_path}.fix-attempt-{attempt_K}.md`
   (e.g. `.planning/phases/11-plan-schema-v2/plans/11-01-PLAN.fix-attempt-1.md`)

### OUTPUT

Write to `{plan_file_path}.fix-attempt-{attempt_K}.md` — NOT to the original plan file.

The orchestrator handles:
- Re-running `validate.cjs` on the sibling file
- Promoting the sibling to overwrite the original on pass
- Deleting the sibling after promotion
- Committing each attempt: `fix({phase}-{plan}): repair schema violation attempt K/3`

### DO NOT

- Overwrite the original `plan_file_path` directly — the orchestrator promotes on pass only.
- Proceed to standard planning steps (break_into_tasks, dependency_graph, etc.).
- Return the standard "PLANNING COMPLETE" report format.
- Change `task.id`, `task.hypothesis`, or `task.files_touched` for any reason.
</fix_schema_mode>
...
- If `--fix-schema` flag present: activate `<fix_schema_mode>` inline section below — do NOT proceed to standard planning steps
...
cat "$phase_dir"/*VTP-ENRICHMENT.md 2>/dev/null   # From Step 6.b.5 enrichment gate (VTPE-01); glob matches {NN}-prefixed per REQ
```
