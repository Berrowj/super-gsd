...
function coordinationFiles(directory) {
  ...
  if (!ownedFile(charter, 128 * 1024) || !ownedFile(assignment, 64 * 1024)) fail('coordination_authority_missing');
  ...
  return { directory, charter, assignment, assignmentValue: value };
}
...
function validCoordinationBinding(value, expected = {}) {
  return value && typeof value === 'object' && !Array.isArray(value)
    ...
    && Object.keys(value).every(key => ['schema_version', 'scope', 'provider', 'accounting_source', 'coordination_id',
      'coordination_dir', 'role', 'pid', 'start_time', 'boot_id', 'executable', 'cwd', 'session_id', 'thread_id'].includes(key))
...
function sameCoordinationBinding(left, right) {
  return ['scope', 'provider', 'accounting_source', 'coordination_id', 'coordination_dir', 'role', 'pid', 'start_time',
  ...
  try {
    ...
    if (run.schema_version !== 1 || run.run_id !== runId || run.scope !== SCOPE || run.provider !== 'openai'
        ...
        || digest(run.coordination_dir) !== run.coordination_id || !validCoordinationBinding(run.native_binding, {
          coordination_id: run.coordination_id, coordination_dir: run.coordination_dir, role: run.role })
        || coordinationFiles(run.coordination_dir).assignmentValue.name !== COORDINATION_NAMES.get(run.role)) return null;
...
function registerCoordination({ root, coordinationDir, role, provider = 'openai', pid, startTime, bootId, executable, cwd, sessionId, threadId }) {
  ...
  const authority = coordinationFiles(coordinationDir);
  if (authority.assignmentValue.name !== COORDINATION_NAMES.get(role)) fail('coordination_assignment_mismatch');
  ...
  const native_binding = { schema_version: 1, scope: SCOPE, provider, accounting_source: ACCOUNTING_SOURCE,
    coordination_id: coordinationId, coordination_dir: authority.directory, role, pid, start_time: startTime,
    boot_id: bootId, executable, cwd: cwd || authority.directory, session_id: sessionId, thread_id: threadId };
  if (!validCoordinationBinding(native_binding, { coordination_id: coordinationId, coordination_dir: authority.directory, role })) fail('invalid_coordination_binding');
  ...
  const run = Object.freeze({ schema_version: 1, scope: SCOPE, provider, accountingSource: ACCOUNTING_SOURCE,
    coordination_id: coordinationId, coordination_dir: authority.directory, role, run_id: runId,
