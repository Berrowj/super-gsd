...
function fixture(t, { parentEnv = process.env } = {}) {
  ...
  return { root, prompt, profileLog: path.join(root, 'profile-resolution.jsonl'), parentEnv };
}
...
test('missing host validator stops before any model call', { skip: process.platform === 'win32' }, t => {
  ...
  { encoding: 'utf8', timeout: 10000, env: { ...f.parentEnv, OPENAI_API_KEY: '', SGSD_ATLAS_DISABLED: '1',
    SGSD_CODEX_PROFILE_LOG: f.profileLog } });
...
test('real wrapper forwards seat model and validates board YAML without a paid call', { skip: process.platform === 'win32' ? 'Run integration under WSL/Linux Bash' : false }, t => {
  const parentRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'board-profile-parent-'));
  ...
  for (const response of [validPosition, 'position: SUPPORT\n']) {
    ...
    const result = spawnSync('bash', ['-c', spec.command], { cwd: f.root, encoding: 'utf8', timeout: 30000,
      env: { ...f.parentEnv, SGSD_CODEX_APP_SERVER_COMMAND: process.execPath, SGSD_CODEX_APP_SERVER_ARGS: JSON.stringify([peer]),
        SGSD_ATLAS_DISABLED: '1', OPENAI_API_KEY: '', SGSD_CIRCUIT_STATE_FILE: path.join(f.root, 'circuit.json'),
        SGSD_CODEX_PROFILE_LOG: f.profileLog, WORKER_FIXTURE_MODE: 'complete', WORKER_FIXTURE_CAPTURE: argvFile,
        WORKER_FIXTURE_REPORT: response } });
  ...
  assert.equal(fs.readFileSync(parentLog, 'utf8'), sentinel, 'ordinary board fixture leaves the inherited parent profile log byte-unchanged');
  ...
  assert.ok(profileRows.some(row => row.action === 'resolve-cli' && row.status === 'fallback'),
    'real board wrapper fallback evidence is written to the fixture-owned profile log');
});
...
test('open provider circuit blocks a board call without substituting its model', { skip: process.platform === 'win32' }, t => {
  ...
  const env = { ...f.parentEnv, SGSD_CODEX_COMMAND: fake, SGSD_ATLAS_DISABLED: '1', OPENAI_API_KEY: '', BOARD_CALLED: called,
    SGSD_CODEX_PROFILE_LOG: f.profileLog };
