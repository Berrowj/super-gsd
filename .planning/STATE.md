...
milestone: v3.4
milestone_name: Operator Cockpit IA Rewrite (Editorial Light) — full IA + liveness contract
milestone_status: "v3.4 ACTIVE 2026-05-25 — Operator Cockpit IA Rewrite. P136 PASS (light palette + IA scaffold; 63/63). P137 PASS (14-key data contract + cockpit-sources.yaml + R19 binding; 71/71). P138 PASS (15s ping + connState reconnect badge + sticky chrome; 79/79). P138.5 fix(): browser-smoke gate landed retroactively. P139 PASS (this commit) — MissionCard + PhaseRunway + AgentLanes + TelemetryRail conformed to design-pack DOM + 73kB canonical CSS extracted from Cockpit.html; self-test 90/90, visual-validate 38/38, browser-smoke 18/18. Cockpit fully rendering with real data (phase 139, success criteria, agent handles, stage status, telemetry sparklines, event tape from git reflog). About to scaffold P140 (§3 Architecture + §4 Milestone SVG diagrams)."
status: "v3.4 P140 PENDING — PhaseArchitectureDiagram + OrchestrationDiagram (§3) + MilestoneStrip + MilestoneDependencyDiagram + PhaseDetailPanel (§4); orthogonal-routed SVG"
