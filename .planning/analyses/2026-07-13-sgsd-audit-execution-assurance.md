...
**OBSERVED.** The checkout contains ten typed profiles, a context registry and packet builder, a detached-worktree executor, host-side patch application checks, independent review contracts, ATC/MUDA/edge/release gates, repair vocabularies, and append-only evidence writers.
...
6. **OBSERVED:** any recognized `profile_override` wins before role mapping, with no trust marker or compatibility check preventing a role from gaining write authority or weakening its worktree, lock, review, or file-count policy (`super-gsd/tools/codex-pro/profile-resolver.cjs:98-116`).
7. **OBSERVED:** the patch profile requires PLAN-LOCKED and max 20 changed files, while its wrapper binds neither constraint and parses patch paths with ad-hoc whitespace-delimited AWK before `git apply` (`super-gsd/registry/codex-profiles.yaml:50-60`; `super-gsd/scripts/codex-patch-executor.sh:23-49`; `super-gsd/scripts/codex-patch-executor.sh:273-316`). **INFERRED:** quoted or rename paths accepted by Git can evade the intended allowlist enumeration.
...
**OBSERVED.** `profile_override` is accepted before every role/phase mapping whenever the named profile exists; the resolver performs no caller-trust or role-compatibility check (`super-gsd/tools/codex-pro/profile-resolver.cjs:98-116`).
...
### F4 — The standalone native-review runner has a wrapper-contract mismatch
...
|---|---|---|---|---|---|---|---|---|---|
| G01 — plan-final review | plan-check returns GO, before plans commit/execution | CONFIGURED direct Codex GPT-5.5/xhigh plan-set ATC+MUDA through `codex-exec.sh`; named flags are OBSERVED supported (`super-gsd/skills/sgsd-orchestrate/SKILL.md:819-838`; `super-gsd/scripts/codex-exec.sh:93-128`) | CONFIGURED `{phaseNum}-PLAN-CODEX-FINAL-REVIEW.md`; live artifact/decision row absent | non-pass redispatches planner; max two loops, then checkpoint | commit plans/proceed or block | no live signal; configured report/branch only | no live latency/tokens | distinct from E06 native runner and E12 post-execution spec review | STRENGTHEN |
...
**OBSERVED.** The gate registry distinguishes trigger, escalation, reviewer, evidence, and repair behavior (`super-gsd/registry/gates.yaml:37-184`). The loader parses/caches rows, soft-warns blocking gates missing repair instructions, rejects repair-command 4-AND violations and invalid sampling tiers, soft-warns missing sampling tiers, and evaluates disabled/trigger behavior through `shouldFire`; it does not perform general gate-shape validation (`super-gsd/scripts/lib/gates-registry.cjs:38-125`).
...
**RECOMMENDED.** The target trace is deliberately separate from current behavior and maps to the non-KEEP entries E01–E12, E14–E15, G01, G04, G06–G08, and G10 in the decision/repair register.

| Node | Trigger / owner | Input | Output / write | Evidence | Failure behavior | Consumer |
|---|---|---|---|---|---|---|
| 1. Resolve transaction | typed Execution Authority | command envelope + task/plan IDs; trusted policy identity | exact role-compatible profile/policy envelope | trace ID and policy hash | unknown, untrusted, privilege-adding, or constraint-weakening override fails closed | packet builder |
...
| 3. Isolate/execute | corrected detached-worktree transaction | packet, target SHA, allowlist | staged + unstaged + untracked change set relative to HEAD | NUL-safe status, base/result tree hashes, report | enumeration/scope/test/process failure discards worktree | host validator |
| 4. Materialize/validate/apply | Git-backed canonical path parser + deterministic host apply | complete change manifest, binary patch/materials, acceptance commands | target tree proven equivalent to accepted isolated tree | patch/material hashes, canonical paths, tree hashes/results | unrepresented change, path/count/lock mismatch, or tree mismatch leaves target unchanged | independent review |
...
| Node | Trigger / owner | Input | Output / write | Evidence | Failure behavior | Consumer |
|---|---|---|---|---|---|---|
| 1. Finding | configured spec-review intent or per-dispatch ATC | PLAN/diff/report/tests | valid `fix_required` or severity finding | review/gate row attempted | spec provider/malformed-output behavior is unbound; ATC follows its configured stop/repair rules | Claude repair branch |
...
| 4. Re-execute | generic executor or patch wrapper | repair prompt/read pack/allowlist | caller-workspace edits or host-applied patch | wrapper output | generic path retains profile/worktree gaps; patch wrapper lacks profile lock/count binding and canonical path enumeration | tests/review |
...
**RECOMMENDED.** Strengthen and merge those mechanisms behind the single entry point in register E03/E04/E05/E09/E10; do not claim registry isolation until it enforces role-compatible profile selection, HEAD-complete change accounting, PLAN-LOCKED, canonical paths, and profile file limits.
...
**DOCUMENTED.** This register reconciles every non-KEEP execution-matrix and assurance-matrix row exactly once. The identifier is the matrix identifier. Findings describe current evidence; owners, dependencies, repairs, risks, rollback paths, and proofs describe the proposed milestone target. KEEP rows E13, G02, G03, G05, and G09 are intentionally absent because their existing mechanism is retained.
...
**INFERRED — RCA (E01–E12, E14–E15):** authority decisions are duplicated across orchestration prose, dispatch router, provider registry, profile YAML, wrappers, double-agent, packet builder, hooks, and reviewers. Their contradictions are currently resolved by call order rather than a typed invariant.

**RECOMMENDED — implementation boundary (E01–E12, E14–E15):** create one local tool/library with two public operations: `resolve --dry-run` and `execute`. Input is command-envelope v1 plus task/plan IDs. Output is report-envelope v2 plus immutable trace ID. It alone may launch delivery Codex and it reuses, rather than duplicates, the existing mechanisms:

- **RECOMMENDED:** profile resolver for exact role policy, with trusted-policy-only monotonic overrides;
- **RECOMMENDED:** context registry/packet/authority for typed context;
- **RECOMMENDED:** double-agent's detached-worktree lifecycle after HEAD-complete staged, unstaged, and untracked accounting is corrected;
- **RECOMMENDED:** Codex executor for the model process and the patch executor's host-side apply pattern after E05 profile, lock, count, canonical-path, and representation enforcement;
...
**RECOMMENDED — acceptance proof (E01–E12, E14–E15):** no production call site invokes `codex` or delivery wrappers outside the authority; policy conformance tests cover all ten exact profiles and reject untrusted or weaker overrides; staged, unstaged, and untracked changes are completely represented and the applied result is tree-equivalent to the accepted isolated result; a trace joins route through repair/closure; pre-apply failure leaves target unchanged; post-apply/pre-evidence failure forces reconciliation/checkpoint; worktree cleanup is proven.
...
**OBSERVED — RCA (E11, E15):** hard-lock policy and availability routing are represented independently, so missing health evidence changes the owner; recognized profile overrides can also replace the mapped role policy without trust or compatibility checks.

**RECOMMENDED (E11, E15):** make authority immutable; availability yields retry/backoff/checkpoint only. Accept profile overrides only from trusted policy and only when every role constraint is preserved or narrowed. Separate Claude synthesis routes from Codex delivery routes in the router schema.
...
### P1 — Native and spec-review integration

**OBSERVED — RCA (E06):** the standalone native runner's selected wrapper/argv mismatch and synthetic self-test do not prove that review boundary.

**CONFIGURED — RCA (E12):** post-execution spec review is a separate read-only orchestration edge, but its exact profile and independent session are not mechanically bound.

**RECOMMENDED (E06, E12):** provide explicitly bound supported entry points for native and spec review, fake-provider end-to-end tests, session-independence checks, and temporary CMB evidence. Keep G01 on its direct plan-final path and bind its decision lifecycle through the separate G01 register repair.
...
- **RECOMMENDED:** keep the patch executor's host-side apply pattern, while replacing its path parsing and adding E05 profile/lock/count enforcement (`super-gsd/scripts/codex-patch-executor.sh:275-314`).
- **RECOMMENDED:** keep the detached-worktree lifecycle from double-agent live mode, while replacing its incomplete diff accounting (`super-gsd/tools/double-agent-executor/run.cjs:584-710`).
...
12. `super-gsd/tools/codex-pro/profile-resolver.cjs`
13. `super-gsd/tools/codex-pro/native-review-runner.cjs`
14. `super-gsd/scripts/codex-executor.sh`
15. `super-gsd/scripts/codex-exec.sh`
16. `super-gsd/scripts/codex-patch-executor.sh`
17. `super-gsd/tools/double-agent-executor/run.cjs`
18. `super-gsd/tools/context-registry/build.cjs`
19. `super-gsd/tools/context-registry/check.cjs`
20. `super-gsd/tools/context-packet/build.cjs`
21. `super-gsd/tools/context-authority/context-anchor-writer.cjs`
22. `super-gsd/tools/dispatch-router/route.cjs`
23. `super-gsd/scripts/lib/gates-registry.cjs`
24. `super-gsd/scripts/lib/gate-value-log.cjs`
25. `super-gsd/scripts/lib/review-ledger.cjs`
26. `super-gsd/scripts/lib/edge-guard.cjs`
27. `super-gsd/tools/phase-verifier/phase-verifier.mjs`
28. `super-gsd/tools/release-readiness/score.cjs`
29. `super-gsd/scripts/sgsd-complete-milestone.cjs`
30. `super-gsd/tools/codex-hooks/enforce-allowed-files.cjs`
31. `super-gsd/tools/codex-hooks/validate-stop-contract.cjs`
32. `super-gsd/tools/chronicle/run-self-test.cjs`
33. `super-gsd/scripts/chronicle-validate.sh`
34. `super-gsd/scripts/sgsd-boot.ps1`
35. `super-gsd/README.md`
...
1. **RECOMMENDED:** every delivery role resolves to exactly one profile and immutable authority; unknown, untrusted, or constraint-weakening `profile_override` values fail closed.
2. **RECOMMENDED:** every `requires_worktree` profile actually runs in a detached worktree.
3. **RECOMMENDED:** every write is constrained by PLAN-LOCKED + profile roots + task allowlist + profile changed-file maximum over deduplicated canonical Git paths.
...
12. **RECOMMENDED:** dry-run resolution and live execution produce the same policy envelope; staged, unstaged, and untracked changes are deterministically materialized; unrepresented changes are rejected; and the validated apply is tree-equivalent to the accepted isolated result.
