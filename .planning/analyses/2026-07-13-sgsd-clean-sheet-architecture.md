...
|---|---|---|---|
| `HUMAN_MANDATE` | Explicit operator decision authenticated to an exact typed aggregate or control subject | Intent acceptance, risk exceptions, policy-required approvals, cancellation, approved journal continuation | Evidence claims it did not independently establish |
...
authoritative_revision: integer # aggregate/control CAS revision; never a journal position
created_at: rfc3339
...
- `TransitionRequest` and `TransitionEvent` require `RUN` or `UNIT`. `JournalContinuationEvent` requires `JOURNAL_GENERATION`; its header `authoritative_revision` is generation-control CAS revision `0`, while its payload `generation_local_revision` is journal position `0`. `SafetyFenceRecord` requires `SAFETY_DOMAIN`; its header revision is the monotonic safety-domain record revision. A consumer rejects any payload/header type disagreement.
- `authoritative_revision` is the signed scope-local CAS revision, never the journal append position: for `RUN` or `UNIT` it is that aggregate's revision; for `JOURNAL_GENERATION` it is the generation control revision (genesis `0`); for `SAFETY_DOMAIN` it is the safety-domain record revision. A producer built from an older revision in its own scope is rejected or explicitly rebased; it is never silently accepted. Separately, `TransitionEvent.revision` and `JournalContinuationEvent.generation_local_revision` are unique, strictly increasing positions inside one journal generation. Two aggregates may both commit aggregate revision `8`, but their events cannot occupy the same generation-local position.
...
`SafetyDomainSubject` is the immutable identity of one fence lineage:

```yaml
SafetyDomainSubject:
  subject_schema: frontier-safety-domain-subject/v1
  safety_domain_id: uuid
  fence_lineage_id: uuid
  topology: PER_RUN|GLOBAL
  affected_journal_generation_ids: [uuid] # non-empty, sorted, unique
  affected_run_ids: [uuid]                # non-empty, sorted, unique
  affected_run_set_hash: lowercase-hex-64
  subject_hash_profile: sha256-jcs-safety-domain-subject-v1
```

`affected_run_set_hash := SHA-256(UTF8("frontier-run-set/v1" + U+0000) || UTF8(JCS(affected_run_ids)))`. Profile `sha256-jcs-safety-domain-subject-v1` calculates `SHA-256(UTF8("frontier-safety-domain-subject/v1" + U+0000) || UTF8(JCS(SafetyDomainSubject)))`; the preimage is exactly the object above and therefore contains neither `control_subject_ref`, its hash value, any fence record, common-header payload/signature hashes, nor a signature. `control_subject_ref` must be `{type: SAFETY_DOMAIN, id: subject.safety_domain_id, hash: calculated_subject_hash}`. A verifier recomputes both hashes and rejects unsorted/duplicate sets, a set/hash disagreement, or a header aggregate ID different from `safety_domain_id`.

The subject hash changes if and only if `safety_domain_id`, `fence_lineage_id`, topology, either affected set, its derived run-set hash, subject schema, or subject-hash profile changes. Record state, timestamps, last-good refs, reason, grant epoch, revoked grants, acknowledgements, recovery refs, predecessor, common-header `authoritative_revision`, payload hash, and signature are mutable record payload and never enter the subject hash. They change only by appending a newly hashed and signed record under safety-domain CAS; no record is edited in place.
...
|---|---|---|---|
| Journal position | Transition Kernel through one append service | `TransitionEvent.revision` is unique and strictly increasing within a generation; aggregate CAS remains the signed header `authoritative_revision` | Position collision/gap or aggregate compare-and-commit failure rejects; producer rebases |
...
| Safety fence | Safety Fence Service | Independent append-only safety-domain CAS chain; one immutable subject per lineage and monotonically increasing grant epoch | Subject/predecessor/revision/epoch conflict denies effects; it cannot authorize work |
...
RecoverySubject:
  ...
  continuation_subject_hash_profile: sha256-jcs-recovery-subject-v1
  continuation_subject_hash: sha-256
```

`RecoverySubjectPreimageV1` is exactly the `RecoverySubject` object above with `continuation_subject_hash` omitted; it retains `continuation_subject_hash_profile`, so the selected profile is signed rather than ambient. Profile `sha256-jcs-recovery-subject-v1` calculates `SHA-256(UTF8("frontier-recovery-subject/v1" + U+0000) || UTF8(JCS(RecoverySubjectPreimageV1)))`. The preimage contains no `control_subject_ref`, `subject.hash`, approval ID/ref/consumption, continuation event hash, common-header payload hash, or signature. The producer calculates the digest from that preimage and only then populates all three equal binding fields: `recovery_subject.continuation_subject_hash`, `subject.hash`, and `subject.control_subject_ref.hash`. A verifier rebuilds the named preimage by omission, never by hashing an object containing or zeroing the digest itself.
...
25. **Revision-domain separation:** signed `authoritative_revision` drives aggregate/control CAS, while a transition event's generation-local `revision` drives journal order and hash chaining; neither value substitutes for the other.
26. **Non-circular control identity:** recovery and safety subject hashes use named, domain-separated JCS preimages that omit the digest being calculated; safety clearance is an immutable successor proven against the exact latest `ACTIVE` predecessor.
...
| Recovery subject hash | Build `RecoverySubjectPreimageV1`, calculate the named digest, and bind it into the three hash fields; then include the digest in its own preimage, zero it, omit the profile, or alter each preimage field | Only omission of `continuation_subject_hash` under `sha256-jcs-recovery-subject-v1` reproduces the signed hash; self-including, zeroed, ambient-profile, and field-altered variants reject before approval consumption |
