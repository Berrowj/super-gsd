...
| Safety subject and clearance | Create an `ACTIVE` fence from canonical `SafetyDomainSubject`, then a `CLEARED` successor; alter one identity field, set/hash order, run-set hash, predecessor envelope/revision/payload hash, state, domain CAS, epoch, continuation scope, approval consumption, or adapter acknowledgement | Canonical ACTIVE verifies; only the same-subject, consecutive-revision/epoch, fully acknowledged successor clears it. Identity changes create a different lineage, mutable in-place edits fail hash/signature proof, and every incomplete/mismatched successor leaves ACTIVE effective |
