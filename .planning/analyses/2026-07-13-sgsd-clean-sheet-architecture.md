...
| `UNIT` | `EVIDENCE_PENDING` | `VERIFYING` | Bundle sealed; each mandatory duty is present or explicitly unavailable | `EvidenceBundle` | Continue collection or request `BLOCKED` at deadline |
...
| `RUN` | `EXECUTING` | `VERIFYING` | No required child is active/repairable/unresolved-blocked and run evidence is sealed | Complete child projection, child verdicts, run evidence bundle | Remain `EXECUTING` |
| `RUN or UNIT` | `VERIFYING` | `SATISFIED` | Signed verdict says every critical claim passes and aggregate rule is true | Aggregate-specific `VerificationVerdict`, evidence bundle | Reject transition |
...
ApprovalEnvelope:
  ...
  subject:
    ...
    aggregate_id: uuid
    run_id: uuid
    kind: CONTRACT|TRANSITION|EFFECT|BUDGET|POLICY|ROLLBACK|APPROVAL
    ref: uuid
    hash: sha-256
    revision: integer
