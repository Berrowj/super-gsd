...
Goal: make a GitHub clone usable by a new SGSD operator without relying on the operator’s local VTP, paths, or prior shell setup.
...
- Replace hard-coded the operator paths in generic MCP onboarding templates with project-dir placeholders.
...
4. Update `super-gsd/templates/onboard/mcp.json.template` to use `{{PROJECT_DIR_FORWARD_SLASH}}` rather than the operator’s absolute checkout path.
