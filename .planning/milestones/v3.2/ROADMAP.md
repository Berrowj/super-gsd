...
| 124 | B | Cockpit research + design lock | Dedicated VTP research pass — pull the chart-redesign figure images (c05f017, c07f034, c05f011 …), audit the current cockpit + cockpit-sidecar.cjs, produce the cockpit design spec. Decides form factor (HTML / terminal / both) with evidence. **Note (2026-05-22): Resonate / Simply Said / Thing Explainer are ingesting into VTP now (operator, ~1h ETA) — by the time the auto-loop reaches P124 the full 7-book base is expected available; the P124 research pass MUST query all 7 books, not just the 4 used in the 2026-05-22 study.** |
