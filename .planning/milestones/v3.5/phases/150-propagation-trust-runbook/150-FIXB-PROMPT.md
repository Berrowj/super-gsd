...
FINDINGS_DETAIL: [CRITICAL] [privacy/no-PII] `PROPAGATION.md:29,88,103-106,121,134-135,148-149` publishes the identifiable Windows account path `C:\Users\jack.berrow\...` throughout runnable commands, violating the locked no-PII publication criterion; derive the path from `$env:USERPROFILE` or accept an operator-supplied root.
...
3. C6: remove EVERY identifiable account path (C:\Users\jack.berrow) from PROPAGATION.md — derive from $env:USERPROFILE / $HOME or operator-supplied root. Grep-verify zero occurrences afterward.
...
## Verify: all six propagation suites green; grep -c 'jack.berrow' PROPAGATION.md -> 0. Report counts.
