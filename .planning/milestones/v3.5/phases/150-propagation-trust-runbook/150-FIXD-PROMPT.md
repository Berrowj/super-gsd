...
## Scope
1. In every tracked file under .planning/milestones/v3.5/phases/150-propagation-trust-runbook/ containing 'jack.berrow' (23 files: plans, research, prompts, reports, reviews): replace the account-path prefix contextually — 'C:\Users\jack.berrow' -> '$env:USERPROFILE' inside PowerShell blocks, 'C:/Users/jack.berrow' -> '$HOME' in bash/MSYS contexts, and '%USERPROFILE%' or a neutral '<redacted-home>' ONLY in pure prose where no execution happens (PROPAGATION.md itself is already clean — do not regress it; its test forbids <...> placeholders in COMMANDS only).
