# Step 9.5 Per-Dispatch ATC — P145 RWFIX diff (codex-exec.sh report-write hardening)

ATC review (delete/simplify/anti-slop + logic/security) of the raw diff below.
Spec compliance passed. Focus: correctness of the 0-byte detection regex
(report_bytes_positive), whether `handle_report_write_failure || true` at the
non-success call sites can mask a byte-count that downstream JSONL needs,
whether exit 9 collides with any existing consumer's exit-code handling
(orchestrator maps 3/4/5/6/8/14), and whether Probe 7's fake-codex harness is
deterministic. Do NOT run self-tests (host: Probes 1-7 PASS, parity PASS).
...
## Report contract (exact — all 5 lines mandatory, contract lines FIRST)
FINDINGS: <int>
CRITICAL: <int>
WARNINGS: <int>
PASS_RATE: <n>/<m>
