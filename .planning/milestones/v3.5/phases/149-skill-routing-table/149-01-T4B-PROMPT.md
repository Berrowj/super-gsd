# P149-T4-continue — COMPLETE the partially-implemented skill-routing consult

A prior dispatch was killed mid-task. Partial state in super-gsd/scripts/lib/orchestrator-hooks.cjs: self-test assertion A10_skill_routing_phase_close_logs_each_decision exists (~line 503-535) and references skillRoutingConsult; the CLI flag --skill-routing-consult is NOT yet wired (usage rejects it); the function may be missing or incomplete. COMPLETE the task: implement/finish skillRoutingConsult + wire the CLI command + make self-test green. Do not restart from scratch — build on what exists.
